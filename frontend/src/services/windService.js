...
const WIND_CONFIG = {
  latMin: 41.2, latMax: 51.5, lonMin: -5.5, lonMax: 9.5,
  step: 1.5, timeout: 30000, maxRetries: 3, retryDelay: 5000
};

let windCache = null, windCacheTime = 0;
...
export const fetchWindData = async () => {
  ...
  const lats = [], lons = [];
  for (let lat = latMin; lat <= latMax; lat += step) lats.push(parseFloat(lat.toFixed(1)));
  for (let lon = lonMin; lon <= lonMax; lon += step) lons.push(parseFloat(lon.toFixed(1)));
  const total = lats.length * lons.length;
  if (total > 50) {
    console.warn(`⚠️ Trop de points (${total}), réduction...`);
    const newStep = 2.0;
    const nlats = [], nlons = [];
    for (let lat = latMin; lat <= latMax; lat += newStep) nlats.push(parseFloat(lat.toFixed(1)));
    for (let lon = lonMin; lon <= lonMax; lon += newStep) nlons.push(parseFloat(lon.toFixed(1)));
    return await fetchWindDataWithRetry(nlats, nlons);
  }
...
const fetchWindDataWithRetry = async (lats, lons, retryCount = 0) => {
  try {
    ...
    if (error.response?.status === 429 && retryCount < WIND_CONFIG.maxRetries) {
      ...
      console.log(`⏳ Rate limit, nouvelle tentative dans ${delay/1000}s...`);
...
const fetchWindDataWithParams = async (lats, lons) => {
  try {
    ...
    let selectedLats = lats, selectedLons = lons;
    const maxPoints = 20;
    if (lats.length * lons.length > maxPoints) {
      const ratio = Math.sqrt(maxPoints / (lats.length * lons.length));
      selectedLats = [];
      selectedLons = [];
      for (let i = 0; i < lats.length; i += Math.max(1, Math.round(1/ratio))) selectedLats.push(lats[i]);
      for (let i = 0; i < lons.length; i += Math.max(1, Math.round(1/ratio))) selectedLons.push(lons[i]);
    }
    ...
    const response = await axios.get(url, { timeout: WIND_CONFIG.timeout, headers: { 'Accept': 'application/json' } });
    if (!response.data || !response.data.hourly) throw new Error('Pas de données horaires');

    const data = response.data;
    const times = data.hourly.time;
    const uData = data.hourly.wind_u_10m;
    const vData = data.hourly.wind_v_10m;
    if (!times || times.length === 0) throw new Error('Aucune donnée temporelle');

    let targetIdx = 0;
    const nowTs = forecastHour.getTime();
    for (let i = 0; i < times.length; i++) {
      const t = new Date(times[i]);
      if (t.getTime() >= nowTs) { targetIdx = i; break; }
    }

    const uValues = uData[targetIdx] || [];
    const vValues = vData[targetIdx] || [];
    const nx = selectedLons.length, ny = selectedLats.length;
    const uGrid = [], vGrid = [];
    for (let j = 0; j < ny; j++) {
      const rowU = [], rowV = [];
      for (let i = 0; i < nx; i++) {
...
export const fetchWindDataSimple = async () => {
  ...
  const points = [
    { lat: 44.0, lon: -1.0 }, { lat: 44.0, lon: 2.0 }, { lat: 44.0, lon: 5.0 },
    { lat: 47.0, lon: -1.0 }, { lat: 47.0, lon: 2.0 }, { lat: 47.0, lon: 5.0 },
    { lat: 50.0, lon: -1.0 }, { lat: 50.0, lon: 2.0 }, { lat: 50.0, lon: 5.0 }
  ];
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    try {
      ...
      if (response.data && response.data.hourly) {
        ...
        allData.push({ lat: p.lat, lon: p.lon, u, v });
  ...
  if (allData.length === 0) throw new Error('Aucune donnée de vent récupérée');
  ...
  const windData = {
    header: {
      nx, ny,
      lo1: lons[0], lo2: lons[lons.length - 1],
      la1: lats[0], la2: lats[lats.length - 1],
  ...
  sessionStorage.setItem(cacheKey, JSON.stringify({ timestamp: Date.now(), data: windData }));
